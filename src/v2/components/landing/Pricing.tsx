          ...
          {tiers.map(t => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${t.isLead ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.08)"}`,
                borderRadius: "var(--le-r-lg)",
              }}
            >
              <div className="le-eyebrow" style={{ marginBottom: 12 }}>{t.name}</div>
              <div className="le-display" style={{ fontSize: 56, lineHeight: 1, marginBottom: 8, color: "#fff" }}>
                {t.priceUsd > 0 ? `$${t.priceUsd.toLocaleString()}` : "Talk"}
              </div>
              <div style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", marginBottom: 24 }}>{t.tagline}</div>
              <ul style={{ listStyle: "none", padding: 0, margin: "0 0 32px", display: "flex", flexDirection: "column", gap: 8 }}>
                {t.features.map(f => (
                  <li key={f} style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontFamily: "var(--le-font-sans)" }}>
                    — {f}
                  </li>
                ))}
              </ul>
              <Link
                to="/upload"
                className={`le-btn ${t.isLead ? "le-btn-primary" : "le-btn-glass"}`}
